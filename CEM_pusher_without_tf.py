...
class CEM():
    ...
    def sample_hori_actions(self, means, vars, samples, elite_indices):
        ...
        if 'CartPole-v0' not in self.env_name:
            ...
            # solution = np.array([np.clip(np.random.normal(means[t], vars[t], self.num_trajs), self.lb, self.ub) for t in range(self.plan_hor*self.action_shape)])
            ...
            # solution = np.array([np.random.normal(means[t], vars[t], self.num_trajs) for t in range(self.plan_hor*self.action_shape)])
            # lb_dist, ub_dist = means - self.lb, self.ub - means
            # constrained_var = np.minimum(np.minimum(np.square(lb_dist / 2), np.square(ub_dist / 2)), vars)
            # constrained_var = np.sqrt(constrained_var)
            # samples = X.rvs(size=[self.num_trajs,self.soln_dim]) * np.sqrt(constrained_var) + means
    ...
    def hori_planning(self, cur_s):
        ...
        if 'CartPole-v0' not in self.env_name:
            ...
            # init_solutions = np.array([np.clip(np.random.normal(means[t], vars[t], self.num_trajs), self.lb, self.ub) for t in range(self.plan_hor*self.action_shape)])
            # lb_dist, ub_dist = means - self.lb, self.ub - means
            # constrained_var = np.minimum(np.minimum(np.square(lb_dist / 2), np.square(ub_dist / 2)), vars)
            # constrained_var = np.sqrt(constrained_var)
            # samples = X.rvs(size=[self.num_trajs,self.soln_dim]) * np.sqrt(constrained_var) + means
            ...
            samples = X.rvs(size=[self.num_trajs,self.soln_dim]) * np.sqrt(constrained_var) + means
            init_solutions = samples.copy().T
            # init_solutions = np.array([np.random.normal(means[t], vars[t], self.num_trajs) for t in range(self.plan_hor*self.action_shape)])
            # lb_dist, ub_dist = means - self.lb, self.ub - means
            # constrained_var = np.minimum(np.minimum(np.square(lb_dist / 2), np.square(ub_dist / 2)), vars)
            # constrained_var = np.sqrt(constrained_var)
            # samples = X.rvs(size=[self.num_trajs,self.soln_dim]) * np.sqrt(constrained_var) + means
        ...
        # print('plan goal',env.ac_goal_pos)
        return best_action

    def get_actual_cost(self, obs):
        to_w, og_w = 0.5, 1.25
        tip_pos, obj_pos, goal_pos = obs[:, 14:17].cpu(), obs[:, 17:20].cpu(), env.ac_goal_pos
        goal_pos = np.repeat(goal_pos.reshape(-1, 1), obs.shape[0], axis = 1).T
        goal_pos = torch.tensor(goal_pos)
        assert isinstance(obs, torch.Tensor)
        ac = np.square(obs[:,20:27].cpu()).sum(dim=1)
    ...
    def get_elites(self, cur_s, sample_hori_actions):
        ...
        for t in range(self.plan_hor):
            ...
            new_pre_ss = np.clip(new_pre_ss, a_min = self.env.observation_space.low[0], a_max = self.env.observation_space.high[0])
        ...
        pre_cum_hori_rewards[np.isnan(pre_cum_hori_rewards)] = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epsilon', type=float, default=0.0001, metavar='NS', help='threshold for cem iteration')
    ...
    parser.add_argument('--collect-step', type=int, default=200, metavar='NS',
                        help='episode number of testing the trained cem')
    parser.add_argument('--lr', type=float, default=0.0001, metavar='T', help='learning rate in training nn.')
    ...
    # cost_model = CostdeepNet(args, obs_shape, action_shape).cuda()
    ...
    # transition function
    my_dx = neural_bays_dx_tf(args, model, "dx", obs_shape, device, sigma2 = 1e-3**2, sigma_n2 = 1e-4**2)
    ...
    # reward function
    # my_cost = neural_bays_dx(args, cost_model, "cost", 1, device, sigma = 0.01, sigma_n = 0.01)
    my_cost = neural_bays_dx_tf(args, cost_model, "cost", 1, device, sigma2 = 1e-3**2, sigma_n2 = 1e-4**2)
    ...
    for episode in range(num_episode):
        ...
        for _ in range(num_steps):
            ...
            # pre_r = cem.get_actual_cost(xu.unsqueeze(0).float())
            # print("real r",r)
            # print("pre r",pre_r)
        ...
        print("my dx mean: {}".format(my_dx.mu_w[:5, :5]))
        print("my dx cov: {}".format(my_dx.cov_w[10, :5, :5]))
